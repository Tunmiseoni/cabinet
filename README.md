...
Spectating has cleared its Phase 0 gate — the RetroArch netplay/spectator spike passed live on the tailnet (2026-09-20: a macOS host + Linux client + Windows spectator stayed synced ~11 min at ~55–83 ms, feel rated acceptable), so RetroArch is the primary spectator path and the `ggponet.dll` shim is skipped. Phase 3 integration follows; the two-concurrent-spectator tailnet run is deferred. Recently landed: a netplay max-ping cap, default-on spectator mute (both live-verified on loopback), an opt-in isolated session config (loopback-retested; default stays off), and an in-app frozen-core download (the `retroarch-cores-v1` release is not published yet). See `docs/04-design.md` §7 and [`docs/07-retroarch-spike.md`](docs/07-retroarch-spike.md) for the open items.
