...
- Training-mode gate (research in [`11-training-mode.md`](11-training-mode.md)): Fightcade's training is the community **peon2/fbneo-training-mode** Lua script (bundled, launched via the `/**training` client command / `quark:training`), and **3rd_training_lua** is a richer 3rd-Strike-only alternative; both are offline-only FBNeo Lua scripts. Open: the exact `quark:training`/`.lua` command-line token order; whether the scripts run unmodified on `sfiii3nr1`; and **licensing** — neither repo declares a license, so neither can be bundled in this public MIT repo. A Cabinet "Practice" launch is gate on a /grill-me session.
...
- Lobby (designed, not built — [`09-lobby.md`](09-lobby.md)): per-ROM RAM result detection is per-game and version-sensitive (core/ROM updates can break it); host-as-spectator is the load-bearing unknown and reverses prior non-goal 07-F18; the control plane is RetroArch's unauthenticated local command socket; no host migration (single point of failure per session); beacon adds a listening socket + per-OS firewall surface. All gated on [`10-lobby-spike.md`](10-lobby-spike.md).
