...
| **Lobby** | Replacement for Phase 2: RetroArch-only rooms (beacon discovery + manual join), first-to-N sets with automatic winner-stays rotation, per-ROM RAM result detection, local per-machine scores/history. Designed in [`09-lobby.md`](09-lobby.md); result detection reads health via `READ_CORE_RAM` (no per-fighter counter exists). **Largely built 2026-09-21:** control-socket plumbing (`providers/retroarch/command.rs`), per-ROM result detection (`lobby/results.rs`), the room model + HTTP beacon (`lobby/room.rs`/`beacon.rs`), hosting, the seat/role split, the client join flow (`lobby_*` commands), and `LobbyCard.tsx` (the preferred home-screen path, with a Stop match control); sets/rotation, the score/history ledger, and `RoomView.tsx` remain. The direct **Launch match** card is now developer-mode only; `lobby_start`/`lobby_join` call the shared `launch_match_inner` while the `launch_match`/`launch_dev_pair` commands stay for that card. | **Build gate (S1–S5) passed locally 2026-09-21**; **release gate [`10-lobby-spike.md`](10-lobby-spike.md) S6–S7 pending** (tailnet soak, beacon reachability) |
