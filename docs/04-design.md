...
| Phase | Work | Gate |
|---|---|---|
| 0 | RetroArch spectator spike: host + 1 client + 2 spectators over Tailscale; measure feel and bandwidth. **Local loopback verified 2026-09-20 (2 players + 2 spectators, parity, spectator no-input, no desync); live <100 ms feel test pending.** See [`07-retroarch-spike.md`](07-retroarch-spike.md) | Feel acceptable at <100 ms? |
...
- Is RetroArch FBNeo netplay feel acceptable for fighting games at <100 ms? **Mechanics proven locally 2026-09-20** (host + client + 2 spectators connect, spectators claim no player slot and send no input, no desync over loopback); only the real tailnet feel remains. See [`07-retroarch-spike.md`](07-retroarch-spike.md).
...
- Redesign / Cabinet mode gate (see [`06-redesign.md`](06-redesign.md)): does the macOS Accessibility grant survive an ad-hoc-signed update, or does frame-follow become the default? Does the macOS emulator window stay above the bezel while remaining clickable? What is the target Lobby/Match information architecture? Is the FightCade dependency worth removing via a RetroArch migration? Does the redesign delay Phase 2.7/3?
...
- RetroArch path: ROM/core parity and content-CRC matching across the four machines, and whether FBNeo core serialization is enabled. **Locally answered 2026-09-20** ([`07-retroarch-spike.md`](07-retroarch-spike.md)): the FBNeo core's `.info` declares `savestate = deterministic`; the frozen macOS/Linux/Windows cores all embed the same revision (`GIT6bb3167`, reported as `v1.0.0.03 GIT6bb3167`); netplay matched content CRC `0x46119843` across all instances; and the TorrentZipped `sfiii3nr1.zip` loads. Cross-OS parity in a live session is still to be confirmed.
