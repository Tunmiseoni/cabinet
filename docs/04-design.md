...
| **Delivery** | A convenient way to get builds onto the other machines and push updates iteratively (see §7) — promoted ahead of further live testing | Friend machines run and update without manual rebuilds |
...
- **Getting builds onto the friend machines is too manual — this is now the active blocker.** Each machine currently has to clone/copy the repo, install OS deps, and run a native build (`scripts/setup-linux.sh`), and there is no way to push updates iteratively — a code change means re-copying and rebuilding by hand, and stale package DBs / missing toolchains derailed the first Linux attempt. **All further live testing (Phase 2.7 and beyond) is deferred until this is solved**, because we cannot iterate on a multi-machine feature by hand-delivering tarballs. Options to evaluate: a prebuilt artifact per OS in a private release (download-and-run), `tauri-plugin-updater` with a signed update feed, a `git pull` + one-command rebuild script, or a package manager. Decide, then resume testing.
