...
- **The fix is now an app.** [`04-design.md`](04-design.md) specifies **cabinet**, a Tauri v2 desktop lobby that wraps `quark:direct`. Phase 1 (launchers, connection health, result watcher, local score ledger) and Phase 2.1–2.6 (rooms/KotH, shared ledger, overlay auto-report, mid-match re-ping) are implemented. The next step is distribution (GitHub Actions → GitHub Releases, with the repo going public after a scrub) plus the Windows launcher adapter. See the root [`README.md`](../README.md) to run it.
