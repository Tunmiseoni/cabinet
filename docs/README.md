...
- **The fix is now an app.** [`04-design.md`](04-design.md) specifies **cabinet**, a Tauri v2 desktop lobby that wraps `quark:direct`. Phase 1 (launchers, connection health, result watcher, local score ledger) and Phase 2.1–2.6 (rooms/KotH, shared ledger, overlay auto-report, mid-match re-ping) are implemented. The next step is distribution (GitHub Actions → GitHub Releases; the repo is now public after a 2026-09-19 history scrub) plus the Windows launcher adapter. See the root [`README.md`](../README.md) to run it.
...
| [`04-design.md`](04-design.md) | Current spec: cabinet Tauri app, network health, KotH, room discovery, score tracking, spectating |
...
| cabinet app | Tauri v2 lobby (Phase 1 + Phase 2.1–2.6: launchers, connection health, rooms/KotH, shared ledger). Run with `scripts/dev.sh`. See root [`README.md`](../README.md). |
