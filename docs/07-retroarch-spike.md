...
| F24 | Sessions only pinned `savefile_directory`/`savestate_directory`, so every other writable path fell back to the host config or RetroArch's own defaults — on macOS several of those are `~/Documents/RetroArch/...` (`system`, `logs`, `playlists`, history/favorites, `records`), which RetroArch recreates on launch. | Pin every writable path per session and move the host data out of Documents. | Med — **Done 2026-09-24:** `write_overrides` now pins `system` (shared, seeded once from the host's `system_directory` so BIOS/samples survive), `cache`, `log_dir`, `playlist_directory`, `content_history_path` + favorites/image/music/video history, `runtime_log_directory`, `screenshot_directory`, and both recording dirs under the app-managed overrides dir; the isolated `base.cfg` pins the shared `system`/`cache` too. Unit tests assert every key lands under the overrides dir and `Documents` appears nowhere. Host migration (macOS): the 12 `~/Documents/RetroArch` keys in the real `retroarch.cfg` were rewritten to `~/Library/Application Support/RetroArch/...` (backup at `retroarch.cfg.bak-20260924`), the `saves`/`playlists` trees moved over, and `~/Documents/RetroArch` deleted. |
